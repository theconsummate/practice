...
def callback_progress(blocks, block_size, total_size, bar_function):
    ...
    if sys.version_info[:3] == (3, 3, 0):  # regression workaround
        if blocks == 0:  # first call
            __current_size = 0
            space_stats = os.statvfs('/')
            available_capacity = (space_stats.f_bavail * space_stats.f_frsize) / 1024
            if total_size > available_capacity/10:
                raise ValueError('total_size > available_capacity/10')
        else:
